...
class PrestoDriverTest extends DriverTests {
  protected override unloadTableEntryName() {
    return 'test_orders_order_status';
  }

  protected override ctasUnloadTableNameTransform(tableName: string) {
    return `memory.default.${tableName}`;
  }

  protected override unloadOptions(tableName: string) {
